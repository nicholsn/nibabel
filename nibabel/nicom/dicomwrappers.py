    ...
    def image_shape(self):
        ...
        try:
            ...
            for frame in self.frames:
                frame_content_seq = frame.get('FrameContentSequence')
                if not frame_content_seq:
                    raise WrapperError("FrameContentSequence is empty.")
                else:
                    dimension_index_vals = frame_content_seq[0].get('DimensionIndexValues')
                    if not dimension_index_vals:
                        raise WrapperError("DimensionIndexValues is empty.")
                    frame_idx_list.append(dimension_index_vals)
            frame_indices = np.array(frame_idx_list)
            # a much more concise approach
            # frame_indices = np.array(frame.FrameContentSequence[0].DimensionIndexValues
            #                          for frame in self.frames])
        except Exception, e:
